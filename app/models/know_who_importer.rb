class KnowWhoImporter
  ...
  def self.create_or_update(know_who_data)
    KnowWhoImporter.new(know_who_data).create_or_update
  end
  ...
  def create_or_update
    ...
    if @leader
      verify_same_state  
      update_attributes_from_know_who
      @leader.save!
    else
      @leader = @state.leaders.new
      @leader.person_id = @data[:pid]
      update_attributes_from_know_who
      @leader.generate_slug # This should happen before_create, but browsercms is overriding
      @leader.publish!
    end
    @leader
  end
  ...
  def leader_exists?
    Leader.exists?(person_id: @data[:pid])
  end
